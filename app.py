...
def get_music_list():
    return [
        ...
        [
            ...
            "https://youtu.be/Hk17wO_mOEU",  # 👈 차단 안 당하는 안전한 주소로 원상복구 완료!
            290,
        ],
        [
...
if idx < len(pool):
    ...
    # 아이폰/PC 크롬 모두에서 완벽 호환되는 포맷(audio/mpeg)과 초 지정 방식 적용
    if st.session_state.audio_url and st.session_state.audio_url != "error":
        ...
        if start_sec > 0:
            st.info(
                f"💡 이 곡은 지정된 하이라이트 구간인 **{start_sec//60}분 {start_sec%60}초** 지점부터 자동 재생됩니다."
            )
